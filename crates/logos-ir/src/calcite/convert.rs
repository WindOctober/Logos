...
use crate::calcite::scalar::{
    collect_scalar_expr, collect_scalar_exprs, split_calcite_arg_list, try_calcite_scalar,
};
use crate::calcite::text_plan::parse_calcite_text_plan;
use crate::calcite::ty::parse_calcite_sql_type;
use crate::calcite::{CalciteFile, CalciteRel};
use crate::error::{Error, Result};
use crate::ir::{
    AggregateCall, Column, Feature, JoinType, LogosIrFile, Query, RelExpr, Schema, SetOp,
    SortDirection, SortKey, SqlType, Table, ValuesTuples,
};
...
fn convert_rel(raw: CalciteRel, features: &mut BTreeSet<Feature>) -> Result<RelExpr> {
    match raw.rel_type.as_str() {
        ...
        "LogicalFilter" => {
            ...
            let condition = required_field("LogicalFilter", "condition", raw.condition)?;
            Ok(RelExpr::Filter {
                input: Box::new(input),
                predicate: try_calcite_scalar(condition)?,
                output: convert_row_type(raw.row_type)?,
            })
        }
        "LogicalJoin" => {
            ...
            let condition = required_field("LogicalJoin", "condition", raw.condition)?;
            Ok(RelExpr::Join {
                left: Box::new(left),
                right: Box::new(right),
                join_type,
                condition: try_calcite_scalar(condition)?,
                output: convert_row_type(raw.row_type)?,
            })
        }
        "LogicalAggregate" => {
        ...
        "LogicalSort" => {
            ...
            Ok(RelExpr::Sort {
                input: Box::new(input),
                collation,
                fetch: raw.fetch.map(try_calcite_scalar).transpose()?,
                offset: raw.offset.map(try_calcite_scalar).transpose()?,
                output: convert_row_type(raw.row_type)?,
            })
        }
