...
(** Predicates have no local runtime-error callback: their ordered arguments
    are checked first.  This structural equality makes explicit that the new
    total predicate can expose exactly child errors and no cast error of its
    own. *)
...
(** Strict DATE < TIMESTAMP uses the same total cross-type comparator, but an
    equal midnight is FALSE. *)
...
(** Strict DATE > TIMESTAMP compares an in-range DATE at midnight and rejects
    equality. *)

(** Non-strict DATE >= TIMESTAMP accepts the same midnight. *)
