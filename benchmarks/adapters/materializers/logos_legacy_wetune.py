"""Frozen source-derived WeTune renderer for campaign-start Logos bytes.

This is a library, not a solver frontend.  A few serialized metadata values
retain their historical spelling because metadata.json belongs to the byte
regression boundary.
"""
...
def materialize_case(
    ...
    with tempfile.TemporaryDirectory(prefix="logos-frozen-wetune-") as tmp:
        tmp_dir = Path(tmp)
...
def parse_table(table_name: str, body: str) -> Table:
    ...
    for item in split_top_level_commas(body):
        ...
        if upper.startswith("PRIMARY KEY"):
            ...
            rest = match.group(2)
            type_sql = normalize_type_for_frozen_legacy(rest)
...
def normalize_type_for_frozen_legacy(type_sql: str) -> str:
...
def build_rename_map(tables: list[Table], query_sql: str) -> dict[str, str]:
    ...
    used = {
        identifier for identifier in identifiers if is_frozen_legacy_safe(identifier)
    }
    rename_map = {}
    for identifier in sorted(identifiers):
        cleaned = identifier.replace('""', '"')
        if is_frozen_legacy_safe(cleaned):
...
def is_frozen_legacy_safe(identifier: str) -> bool:
    return bool(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", identifier)) and (
        identifier.lower() not in FROZEN_LEGACY_RESERVED
    )
...
def make_safe_identifier(identifier: str) -> str:
    ...
    if candidate.lower() in FROZEN_LEGACY_RESERVED:
