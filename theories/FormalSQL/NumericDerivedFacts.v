...
From SQLFS Require Import Bool3 SqlErrorSemantics SqlSyntax SqlQuerySyntax
  SqlQuerySemantics FTuples Interp.

(** The typed disequality law lifts to arbitrary successfully evaluated
    INTEGER terms.  Term interpretation and runtime safety remain explicit,
    so callers may instantiate attributes, constants, casts, or expressions
    without fixing a generated query shape. *)
...
Proof.
intros basesort instance value_is_null env subject first_term second_term
  value first second Hsubject Hfirst Hsecond
  Hsubject_safe Hfirst_safe Hsecond_safe Hunequal outcome.
assert (Hleft_safe :
  first_runtime_error
    (@eval_aggterm_runtime_error TNull
      NullValues.interp_scalar_operator_runtime_error
      NullValues.interp_aggregate_runtime_error env)
    [subject; first_term] = None).
{ cbn; now rewrite Hsubject_safe, Hfirst_safe. }
assert (Hright_safe :
  first_runtime_error
    (@eval_aggterm_runtime_error TNull
      NullValues.interp_scalar_operator_runtime_error
      NullValues.interp_aggregate_runtime_error env)
    [subject; second_term] = None).
{ cbn; now rewrite Hsubject_safe, Hsecond_safe. }
split.
- intro Heval.
  ...
  + match goal with
    ...
    rewrite Hsubject, Hfirst, Hsecond.
    f_equal.
    now apply interp_int32_neq_disjunction_true_of_unequal_constants.
- intro Houtcome; subst outcome.
  ...
  rewrite <- (interp_int32_neq_disjunction_true_of_unequal_constants
    value first second Hunequal).
  exact Heval.
Qed.
