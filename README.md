...
- `vendor/FormalSQL`: a Rocq-modernized fork of SQLCoq/SQLFormalSemantics. Logos uses one compositional normalized query syntax with an exact ordered-outcome relation, together with reusable bag operators and a proved possible-bag abstraction.
...
The exact evaluator remains an outcome relation over ordered row lists at every nesting depth. For a `BagClosed` region, every possible bag can recover any requested row order through an actually evaluated, `ordered_rows_equiv` result, so `alpha` is complete without requiring a particular hidden Rocq tuple representation. Proofs may then use lifted bag operations. Reset-derived permutation closure composes through projection, row mapping, and filtering; in particular, the relational expansion `Filter (CrossJoin left right)` directly has the permutation-closed SQL observation expected of an inner join.
...
`manifest.json` records every indexed public `Lemma`, `Theorem`, and
`Corollary` with its exact ownership source, line, and declaration.
...
`EXP(AVG(integer))` retains both the exact PostgreSQL NUMERIC average and its runtime-selected display scale. A declarative row map applies one deterministic `NumericExpModel` to each non-NULL average; NULL bypasses the model, successful results must be finite PostgreSQL-storable NUMERIC values already rounded at a valid display scale, and overflow is SQLSTATE 22003. Generated source and target programs take the same universally quantified model. Equivalence must therefore hold for every valid deterministic EXP implementation, including PostgreSQL's, without an axiom, a chosen witness, or a planner-dependent evaluation schedule.
...
`SqlQueryWellFormed.v` checks the post-desugaring Rocq core directly, without introducing a second isomorphic query AST. Every generated and independently trusted certificate goal requires both ordered query programs to satisfy structural `query_expr_admissible` under each conforming database, including unique projection outputs, disjoint cross-join schemas, ordered set compatibility, grouping metadata, window attributes, and positional subqueries. Each immutable generated `Queries.v` must also compile the stronger `TNullQueryExprTypedNativeScalarAdmissibleWithOutputs` certificate, which adds native scalar witness, operator-arity/type, phase, exact-output, and analysis-error-placement checks before projecting the structural fact used by `Goal.v`; an externally attested `QExpr_Error` is legal only at the root and therefore cannot be hidden by target elimination or `FETCH 0`. Calcite and Rust remain trusted for translation into that core. The Rust validator runs after lowering and again at emission, where it additionally checks exact PostgreSQL NUMERIC versus `DECIMAL(p,s)` result shapes instead of treating their typmods as interchangeable; Rocq then checks the generated certificate. Together these boundaries prevent a malformed emitted term from bypassing validation.
